# This Makefile is for the IMacros extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/IMacros.pm]
...
IMacros.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"IMacros\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
subdirs ::
	$(NOECHO) cd Util && $(MAKE) $(USEMAKEFILE) $(FIRST_MAKEFILE) all $(PASTHRU)


# --- MakeMaker clean_subdirs section:
clean_subdirs :
	$(ABSPERLRUN)  -e "exit 0 unless chdir 'Util';  system '$(MAKE) clean' if -f '$(FIRST_MAKEFILE)';" --
...
realclean_subdirs : clean
	- $(ABSPERLRUN)  -e "chdir 'Util';  system '$(MAKE) $(USEMAKEFILE) $(MAKEFILE_OLD) realclean' if -f '$(MAKEFILE_OLD)';" --
	- $(ABSPERLRUN)  -e "chdir 'Util';  system '$(MAKE) $(USEMAKEFILE) $(FIRST_MAKEFILE) realclean' if -f '$(FIRST_MAKEFILE)';" --
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "name: IMacros" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
	$(NOECHO) $(ECHO) "   \"name\" : \"IMacros\"," >> META_new.json
...
subdirs-test_dynamic :: dynamic pure_all
	$(NOECHO) cd Util && $(MAKE) test_dynamic $(PASTHRU)
...
subdirs-test_static :: static pure_all
	$(NOECHO) cd Util && $(MAKE) test_static $(PASTHRU)
...
ppd :
	$(NOECHO) $(ECHO) "<SOFTPKG NAME=\"IMacros\" VERSION=\"0.01\">" > IMacros.ppd
	$(NOECHO) $(ECHO) "    <ABSTRACT>Perl extension for blah blah blah</ABSTRACT>" >> IMacros.ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>A. U. Thor &lt;a.u.thor@a.galaxy.far.far.away&gt;</AUTHOR>" >> IMacros.ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> IMacros.ppd
	$(NOECHO) $(ECHO) "        <ARCHITECTURE NAME=\"MSWin32-x86-multi-thread-64int-5.24\" />" >> IMacros.ppd
	$(NOECHO) $(ECHO) "        <CODEBASE HREF=\"\" />" >> IMacros.ppd
	$(NOECHO) $(ECHO) "    </IMPLEMENTATION>" >> IMacros.ppd
	$(NOECHO) $(ECHO) ^</SOFTPKG^> >> IMacros.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e "pm_to_blib({{@ARGV}}, '$(INST_LIB)\auto', q[$(PM_FILTER)], '$(PERM_DIR)')" -- \
	  REAMDE.pod $(INST_LIB)\REAMDE.pod \
	  lib/IMacros.pm blib\lib\IMacros.pm 
